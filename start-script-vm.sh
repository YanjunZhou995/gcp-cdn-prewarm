...
# 成功URL的本地记录文件。
readonly SUCCESS_LOG="${RESULTS_DIR}/success.log"
# 失败URL的本地记录文件。
readonly FAILURE_LOG="${RESULTS_DIR}/failure.log"
...
# --- 主要逻辑 ---
# 1. 从GCS下载URL列表
#    注意：请在Pulumi代码中将 {gcs_url_list_path} 替换为实际的GCS路径。
log_to_gcp "Starting URL processing script." "INFO"
gsutil cp gs://zyj-paper-cdn/url-shards/10w_urls.txt "$URL_LIST_FILE"
if [ $? -ne 0 ]; then
...
log_to_gcp "URL list downloaded successfully." "INFO" '{"vm_name": "'$(hostname)'", "total_urls": '$TOTAL_URLS'}'
...
process_url() {
    ...
    if [[ "$http_code" =~ ^[23] ]]; then
        ...
        log_to_gcp "Request failed for URL." "WARNING" '{"vm_name": "'$(hostname)'", "url": "'"$url"'", "http_code": '"$http_code"'}'
    fi
}
...
#    --eta: 显示预计完成时间。
log_to_gcp "Starting parallel processing of URLs..." "INFO" '{"concurrent_jobs": 1000}'
cat "$URL_LIST_FILE" | parallel -j 100 --eta process_url
...
log_to_gcp "Cleanup complete. Script finished." "INFO" '{"vm_name": "'$(hostname)'"}'
